"""TP sigma sweep on b30_35 paired with fh.

Methodology: anchor fh at sigma=0.75 (known production setting),
sweep b30_35 sigma across {0.0, 0.25, 0.5, 0.75, 1.0, 1.25}.
Run fh + b30_35 (no ft, no other buckets) — pair structure mirrors
how ft's optimal sigma=0.0 was originally discovered.

Find b30_35's best sigma in fh-pair context. That sigma value then
goes into multi-bucket (fh + ft + b30_35 tuned) config later.
...
SWEEP_DIR = REPO / "logs" / "sigma_sweep_b30_35"
...
def make_cfg(sigma: float, cfg_path: Path) -> None:
    """fh + b30_35 pair, fh fixed sigma=0.75, b30_35 sigma=sweep value."""
    cfg = json.loads(BASE_CONFIG.read_text())
    fh = copy.deepcopy(cfg["buckets"][0])  # fish_head_production with sigma=0.75
    b30_35 = copy.deepcopy(fh)
    b30_35["strategy_id"] = "fish_head_b30_35"
    b30_35["label"] = "fish_head_b30_35"
    b30_35["sigma"] = sigma
    cfg["buckets"] = [fh, b30_35]
...
def build_report(results: dict) -> str:
    lines = [
        "# fh + b30_35 sigma sweep (fh fixed 0.75, b30_35 swept)",
        "",
        "## Total comparison",
        "",
        "| b30_35 sigma | Trades | WR | P/L | CAGR | MDD | Calmar | NegYrs |",
        "|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for sigma in SIGMAS:
        r = results.get(sigma, {})
...
def main() -> int:
    print(f"[sigma_sweep_b30_35] Start {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    results: dict = {}
    for idx, sigma in enumerate(SIGMAS, 1):
        ...
        if is_done(log_path):
            print(f"[sigma_sweep_b30_35] [{idx}/{len(SIGMAS)}] σ={sigma:.2f} skip (done)", flush=True)
        else:
            make_cfg(sigma, cfg_path)
            t0 = time.time()
            print(f"[sigma_sweep_b30_35] [{idx}/{len(SIGMAS)}] σ={sigma:.2f} start", flush=True)
            rc = run_sim(cfg_path, log_path)
            elapsed = time.time() - t0
            print(
                f"[sigma_sweep_b30_35] [{idx}/{len(SIGMAS)}] σ={sigma:.2f} "
    ...
    print(f"[sigma_sweep_b30_35] Done {time.strftime('%Y-%m-%d %H:%M:%S')}", flush=True)
    return 0
