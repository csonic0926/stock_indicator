...
How the system goes from **SEC `company_tickers_exchange.json`** to a stable, FF12-classified
**tradable universe** (`symbols.txt` + `symbols_with_sector.parquet`).

Yahoo Finance price download is **out of scope** here — it consumes the output
of this pipeline. See `daily_job.update_all_data_from_yf` for that.
...
| File | Meaning |
|---|---|
| `data/symbols.txt` | Tradable universe contract. Newline-separated. Runtime trusts this exactly. |
| `data/symbols_with_sector.parquet` | Same universe + `cik`, `sic`, `ff12`, `ff48`, `ff49`, `ff_label`, `sic_desc`, `ff12_source` columns. Used for sector-aware Pick-N. |
| `data/symbols_with_sector.csv` | Human-readable mirror of parquet. |
...
Runtime rule: any symbol in `symbols.txt` **must** have a row in the parquet
with a non-null `ff12` (1-12). Coverage is **100%**.
...
**Purpose**: restore the important behavior that the old `2010_safe` universe had implicitly: a symbol must not be admitted just because it appears in SEC reporting data. It must also look like a primary-exchange tradable listing with enough local daily history for the strategy mechanism to be meaningful.
...
- This is intentional for explicit research exceptions; those symbols remain auditable as `tradability_status=manual_include`.
...
- This is the key difference from the bad 7.4K new universe: SEC-reporting status alone is no longer enough.
...
Production cron is locked to the frozen old universe.  The research universe
pipeline is intentionally not part of `run_daily_job.sh`, because it must not
overwrite production `symbols.txt` / `symbols_with_sector.*` before the daily
price refresh.

```bash
# === Phase A: Price data (consumes production-old symbols.txt) ===
...
|---|---|---|
| Production daily cache | Daily | Uses frozen production-old `symbols.txt` |
| Production daily signal | Daily, after price refresh | Uses `data/multi_bucket_production.json` |
| Research universe refresh | Manual / research cron only | Publishes `research_new_*` aliases and must not change production files |
...
- Never edit production `symbols.txt` / `symbols_with_sector.parquet` in place
- Build to temp files, then `mv` atomic
- Research refresh writes `research_new_symbols.txt` and
  `research_new_symbols_with_sector.*`

### Pre-swap validation criteria

Phase A validates the candidate contract before any atomic replace:

| Check | Rule | Why |
|---|---|---|
| Symbol count drift | Proposed research universe must not drop by more than 5% versus current `research_new_symbols.txt` | Catches half-broken SEC responses that return only a partial ticker table |
| Tradability gate | Included symbols must pass trusted SEC exchange + mature local history, unless manually policy-included | Blocks SEC-reporting OTC/foreign/IPO-ish noise from entering cron output |
| Sector row count | `research_new_symbols_with_sector` ticker set must exactly equal proposed `research_new_symbols.txt` | Prevents runtime symbols with no FF12 row |
...
large removals require manual audit. For the one-time migration that activates
the tradability gate against the bad 7.4K universe, run with an explicit larger
threshold such as `--maximum-drop-ratio 0.50`; after that, return cron to the
default 5% guard because normal daily churn should be tiny.

### Failure handling

| Failure | Behavior |
|---|---|
| SEC API down / timeout | Research refresh aborts; production remains on old symbols |
| LLM API down / rate limit | Stage 4 aborts; existing classifications retained; new symbols stay quarantined until next run |
| yf cumulative failure | Production daily cache refresh is partial; signal generation uses whatever CSVs exist |
| Validation / parquet rebuild fail | Research refresh aborts; existing research files retained |

**Fail-closed for new symbols**: if a new ticker can't be classified (LLM fails), keep it OUT of `research_new_symbols.txt` until classified. Don't default-include new noise.

### Cron implementation status

- `run_daily_job.sh` does not call `update_universe_pipeline`
- The research management command is `python -m stock_indicator.manage update_universe_pipeline`
- Controlled research refreshes may use `python -m stock_indicator.manage update_universe_pipeline --dry-run --maximum-drop-ratio 0.50`
- Last `update_sector_data` run: see `cache/submissions/*.json` mtimes; latest = 2026-04-08

### Promotion concern (separate from cron design)

The new universe is research-only until explicitly promoted.  Before any future
promotion, pre-warm or reset `adaptive_state.json` deliberately so newly added
symbols do not silently enter production with mismatched rolling state.
- Or accept cold-start TP/SL inaccuracy on ~3,000 symbols as bounded one-time cost?

This is **out of scope for the universe pipeline** but blocks safe production cutover.
